...
| Item | Details |
|------|---------|
| Create Task Endpoint | `POST https://app-cumnzg2pfz0h-api-GYX1bbkRQj4a.gateway.appmedo.com/v1/images/editing/expand` |
| Query Task (Single) Endpoint | `GET https://app-cumnzg2pfz0h-api-AalZkkAG5w7L.gateway.appmedo.com/v1/images/editing/expand/{task_id}` |
| Query Task (List) Endpoint | `GET https://app-cumnzg2pfz0h-api-pLVzAAkGZwDL.gateway.appmedo.com/v1/images/editing/expand` |
...
async function createExpandTask(params: {
  ...
  const response = await fetch(
    "https://app-cumnzg2pfz0h-api-GYX1bbkRQj4a.gateway.appmedo.com/v1/images/editing/expand",
    {
      method: "POST",
      headers: {
...
async function queryExpandTask(taskId: string): Promise<{
  ...
  const response = await fetch(
    `https://app-cumnzg2pfz0h-api-AalZkkAG5w7L.gateway.appmedo.com/v1/images/editing/expand/${taskId}`,
    {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
